...
const runAnimation = () => {
    mainTextTitles.forEach(mainTextTitle => {
        if(elInView(mainTextTitle, 100))
        {
            addAnimation(mainTextTitle)
        }
    })

    mainTextParagraphs.forEach(mainTextParagraph => {
        if(elInView(mainTextParagraph, 100))
        {
            addAnimation(mainTextParagraph)
        }
    })

    imgContainers.forEach(imgContainer => {
        if(elInView(imgContainer, 100))
        {
            addAnimation(imgContainer)
        }
    })
};
